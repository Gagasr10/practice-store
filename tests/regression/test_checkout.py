...
def _reach_address_step(user_page: Page) -> CheckoutPage:
    ...
    # Step 2 (sign-in): authenticated users may auto-advance past this step.
    # Wait for either proceed-2 or the address-step country field, then click
    # proceed-2 only if it is present.
...
def test_unauthenticated_user_redirected_to_login_at_checkout(page: Page):
    ...
    # Navigate to checkout via the cart icon (SPA nav) so Angular initialises before
    # hitting the auth-guarded steps.
    page.locator("[data-test='nav-cart']").click()
    page.wait_for_url("**/checkout**", timeout=10_000)
    cart = CartPage(page)
    expect(cart.proceed_button).to_be_visible(timeout=20_000)
    cart.proceed_to_checkout()
    # Step 2 (SIGN IN) requires authentication — the email input is now visible.
    auth = AuthPage(page)
